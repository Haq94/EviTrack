...
belief_plot.py
===============
Standalone script: seed-aggregated DD-aligned belief diagnostic plots + pre/post tables.
Belief metrics have no horizon dimension.
Produces one figure per metric, with 1 x n_bins subplots (one per DD bin).
Run directly: python belief_plot.py
"""
...
# =============================================================================
# METRIC CONFIGS
# NOTE: entropy is already normalized to [0,1] by log(K) at compute time.
#       ess is raw (1 to K); set ylim or clip if you want to normalize display.
# =============================================================================
METRIC_CONFIGS = {
    "ess":     {"key": "ess",     "name": "Effective Sample Size", "ylabel": "ESS",              "clip": None, "ylim": None,        "transform": None},
    "entropy": {"key": "entropy", "name": "Normalized Entropy",    "ylabel": "Entropy",          "clip": None, "ylim": (0.0, 1.0),  "transform": None},
    "ba_filt": {"key": "ba_filt", "name": "Filtering Branch Acc.", "ylabel": "Branch Accuracy",  "clip": None, "ylim": (0.0, 1.0),  "transform": None},
}
...
def apply_transform(arr: np.ndarray, transform: Optional[str]) -> np.ndarray:
    if transform == "square":
        return arr ** 2
    return arr
...
def apply_neurips_style(ax, pc: dict):
    ...
    if pc["show_grid"]:
        ...
        ax.grid(True, alpha=pc["grid_alpha"], linewidth=pc["grid_linewidth"],
                linestyle=pc["grid_linestyle"], color="#aaaaaa")
...
def compute_pre_post_table(
    data_by_traj_seed, engines, seed_indices, traj_indices, metric_key, transform
) -> pd.DataFrame:
    rows = []
    for engine in engines:
        ...
        for seed in seed_indices:
            ...
            for traj_idx in traj_indices:
                ...
                arr     = apply_transform(np.asarray(metrics[metric_key], dtype=float), transform)
...
def compute_agg_curves(
    data_by_traj_seed, engines, seed_indices, traj_indices, metric_key, transform, window
) -> Dict[str, dict]:
    agg = {}
    for engine in engines:
        seed_centers = []
...
def plot_bins(
    ...
    for i, bin_label in enumerate(bin_order):
        ...
        for engine in engines:
            ...
            ax.fill_between(t_rel, lower, upper,
                            alpha=pc["band_alpha"], color=style["color"],
                            linewidth=0, zorder=2)

        ax.axvline(0, color=pc["dd_line_color"], linestyle=pc["dd_line_style"],
                   linewidth=pc["dd_line_width"], alpha=pc["dd_line_alpha"], zorder=4)

        apply_neurips_style(ax, pc)
    ...
    if pc["show_suptitle"]:
        fig.suptitle(metric_name, fontsize=pc["suptitlesize"], y=1.02)

    plt.tight_layout(pad=pc["tight_layout_pad"], w_pad=0.4)
    fig.subplots_adjust(bottom=pc["bottom_adjust"])
...
def main():
    ...
    # Belief metrics have no horizon dimension — always organize at horizon_idx=0
    loader = MetricLoader(replay_dir=replay_dir, verbose=True)
    loader.load().organize(horizon_idx=0)

    engines = preferred_engine_order(
        select_available_indices(ENGINE_NAMES, loader.engines, "engines"),
        ENGINE_ORDER_PREFERRED,
    )
    seeds  = list(loader.all_seed_indices)
    trajs  = list(loader.all_traj_indices)
    data_by_traj_seed = loader.data_by_traj_seed

    engines = [e for e in engines if e in loader.engines]
    seeds   = select_available_indices(seeds, loader.all_seed_indices, "seeds")

    bin_to_trajs_all = stratify_by_dd_time(
        data_by_traj_seed, engines, trajs, seeds, bin_ranges_all
    )
    ...
    for metric_key_name in METRICS:
        ...
        print("-" * 60)
        df = compute_pre_post_table(data_by_traj_seed, engines, seeds, trajs, metric_key, transform)
        ...
        for label in bin_order:
            ...
            df_bin = compute_pre_post_table(
                data_by_traj_seed, engines, seeds, trajs_bin, metric_key, transform
            )
        ...
        for label in bin_order:
            ...
            bin_agg_curves[label] = compute_agg_curves(
                data_by_traj_seed, engines, seeds, trajs_bin, metric_key, transform, WINDOW
            )

        # --- PLOT ---
        save_stem = Path(SAVE_DIR) / "belief_diagnostic_plots" / f"{FIGURE_NAME}_{metric_key_name}_bins"
        plot_bins(
